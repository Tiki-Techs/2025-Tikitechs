...
/** Reports our expected, desired, and actual poses to dashboards */ // ADD A THING TO FULLY TRUST VISION WHEN
                                                                     // DISABLED!!!!!!!!
public class PoseEstimator extends SubsystemBase {
    ...
    public static PoseEstimator getInstance() {
        if (instance == null)
            instance = new PoseEstimator(); // Need to take swerve out of constructor and use the getInstance originally
                                            // used.
        return instance;
    }
    ...
    public void periodic() {
        ...
        isAuto = DriverStation.isAutonomous() || DriverStation.isAutonomousEnabled();
        ...
        if (estimateFrontPose != null) { // Limelight mode; originally had a boolean for whether limelight was true or
                                         // false, but 3880 only runs limelight (2024-2025)
            double currentTimestamp = RobotContainer.m_vision
                    .getTimestampSeconds(RobotContainer.m_vision.getTotalLatency("limelight-front"));
            if (isEstimateReady(estimateFrontPose)) { // Does making so many bot pose variables impact accuracy?
                if (!isDisabled && !isAuto) {
                    // if (!isDisabled) {
                    poseEstimator.setVisionMeasurementStdDevs(createVisionMeasurementStdDevs(2, 2, 30000000));

                    frontChanged = true;
                    // poseEstimator.setVisionMeasurementStdDevs(RobotContainer.m_vision.modVisionStdDevsF(2.5,
                    // 2.5));
                    addVisionMeasurement(estimateFrontPose, currentTimestamp);
                } else {
                    // poseEstimator.setVisionMeasurementStdDevs(createVisionMeasurementStdDevs(500000000.5,
                    // 500000000.5, 30000000));
                    // addVisionMeasurement(estimateFrontPose, currentTimestamp);
                    frontChanged = false;
                }
            }
        ...
        if (estimateBackPose != null) {
            double currentTimestamp = RobotContainer.m_vision
                    .getTimestampSeconds(RobotContainer.m_vision.getTotalLatency("limelight-back"));
            if (isEstimateReady(estimateBackPose)) {
                if (!isDisabled && !isAuto) {
                    // if (!isDisabled) {
                    poseEstimator.setVisionMeasurementStdDevs(createVisionMeasurementStdDevs(1.5, 1.5, 30000000));

                    backChanged = true;
                    // poseEstimator.setVisionMeasurementStdDevs(RobotContainer.m_vision.modVisionStdDevsB(2.5,
                    // 2.5));
                    addVisionMeasurement(estimateBackPose, currentTimestamp);
                } else {
                    // poseEstimator.setVisionMeasurementStdDevs(createVisionMeasurementStdDevs(5000000000.5,50000000000.5,
                    // 30000000));
                    // addVisionMeasurement(estimateBackPose, currentTimestamp);
                    backChanged = false;
                }
        ...
        // if (VisionConfig.IS_PHOTON_VISION_MODE && estimatePose != null) { //
        // Limelight mode
        // double photonTimestamp = Vision.getInstance().getPhotonTimestamp();
        // if (isEstimateReady(estimatePose)) { // Does making so many bot pose
        // variables impact accuracy?
        ...
        if (frontChanged || backChanged) { // Boolean for whether or note vision override is enabled. Assumption is that
                                           // this will just be used when robot is disabled.
            drivetrain.swerveDrive.resetOdometry(getPosition());
        }

        // telemetry.updatePoseOnField("VisionPose", Robot.vision.botPose);
        // telemetry.updatePoseOnField("OdometryPose", odometryPose); // Was not
        // commented
    ...
    // /**
    // * @return true if estimated pose is on the chargestation by using the
    // field-space
    // chargestation
    // * dimensions
    // */
    ...
    // /**
    // * Returns the most accurate pose. If we are not confident that vision is
    // accurate,
    ...
    /**
     * Helper method for comparing vision pose against odometry pose. Does not
     * account for difference
     * in rotation. Will return false vision if it sees no targets or if the vision
     * estimated pose is
    ...
    public boolean isEstimateReady(Pose2d pose) {
        ...
        if (RobotContainer.m_vision.visionAccurate(pose)) { // visionAccurate method sees if Apriltags present in
                                                            // Vision.java
            return true;
        }
