...
public class SwerveSubsystem extends SubsystemBase {
  ...
  // " if (instance == null) instance = new SwerveSubsystem(new
    ...
    AutoBuilder.configure(
        ...
        (speeds, feedforwards) -> {
          if (enableFeedforward) {
            ...
            // unsure on this, but is what is in the other team's code as well as a command
            // built into swerve drive
          } else {
  ...
  public Command driveCommandF(DoubleSupplier translationX, DoubleSupplier translationY,
    ...
    return run(() -> {
      ...
      swerveDrive.driveFieldOriented(swerveDrive.swerveController.getTargetSpeeds(
          scaledInputs.getX(),
          scaledInputs.getY(),
          swerveDrive.getOdometryHeading().getRadians() + angularRotationX.getAsDouble() * 1.7,
          swerveDrive.getOdometryHeading().getRadians(),
          swerveDrive.getMaximumChassisVelocity()));
    });
  }
   ...
   * @param relativePos The relative position on the reef (-1 for right branch, 0
   *                    for center, 1 for left branch).
  ...
  public void periodic() {
    ...
    // );
    closestTag();
    robotPose = new Pose3d(swerveDrive.getPose().getX(), swerveDrive.getPose().getY(), 0,
    ...
    // SmartDashboard.putNumber("Encoder Front Right relative",
    // swerveDrive.getModules()[1].getRelativePosition());
    // SmartDashboard.putNumber("Distance from 20", getDistanceFromAprilTag(20));
    // SmartDashboard.putNumber("Odometry",
    // swerveDrive.getOdometryHeading().getDegrees());
    // SmartDashboard.putNumber("gyro test",
    // swerveDrive.getGyroRotation3d().getAngle());
